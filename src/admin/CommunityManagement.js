...
import { PopupDevComAdd, PopupDevComUpdate } from './Popup';
import { toast } from 'react-toastify';
import Spinner from 'react-bootstrap/Spinner';
import { UpdateCommunity } from '../api/Api';

export const CommunityManagement = ({ config }) => {
    ...
    const handleSave = async () => {
        ...
        const response = await UpdateCommunity(data);
        window.scrollTo(0, 0);
    ...
    return (
        <>
            ...
            {
                popup.update ? <PopupDevComUpdate array={dArray} setArray={setDArray} update={popup.update} setUpdate={setPopup} obj={selected.obj} i={selected.index} /> : <></>
            }
            {
                popup.add ? <PopupDevComAdd array={dArray} setArray={setDArray} update={popup.add} setUpdate={setPopup} /> : <></>
            }
        </>
    );
}